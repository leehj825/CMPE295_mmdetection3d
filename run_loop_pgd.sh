...
#SBATCH --output=log.loop_pgd_2.log
#SBATCH --ntasks=1
#SBATCH --nodes=1
...
do
    ...
    do
        if [ $i -eq 100 ]; then
            SCRIPT_NAME="run_pgd_waymo_n$j.sh"
        else
            SCRIPT_NAME="run_pgd_waymo_n${j}_resume.sh"
        fi
    ...
    do
        ...
        SBATCH_OUTPUT=$(sbatch --parsable $SCRIPT_NAME)
        JOB_ID=$(echo $SBATCH_OUTPUT | awk '{print $NF}')
        THIRD_JOB_IDS+=($JOB_ID)
        echo "Started job $JOB_ID with script $SCRIPT_NAME"
    done

    MINUTE_COUNTER_5_6=0
    # Wait for the first two jobs to complete
    for JOB_ID in ${THIRD_JOB_IDS[@]}
    do
        while squeue | grep -wq "$JOB_ID"; do
            if (( MINUTE_COUNTER_5_6 % 10 == 0 )); then
                CURRENT_TIME=$(date +%s)
                echo "Checked at $(date -d @$CURRENT_TIME): job $JOB_ID is still running..."
            fi
            sleep 60
    ...
    do
        ...
        SBATCH_OUTPUT=$(sbatch --parsable $SCRIPT_NAME)
        JOB_ID=$(echo $SBATCH_OUTPUT | awk '{print $NF}')
        FOURTH_JOB_IDS+=($JOB_ID)
        echo "Started job $JOB_ID with script $SCRIPT_NAME"
    done

    MINUTE_COUNTER_7_8=0
    # Wait for the first two jobs to complete
    for JOB_ID in ${FOURTH_JOB_IDS[@]}
    do
        while squeue | grep -wq "$JOB_ID"; do
            if (( MINUTE_COUNTER_7_8 % 10 == 0 )); then
                CURRENT_TIME=$(date +%s)
                echo "Checked at $(date -d @$CURRENT_TIME): job $JOB_ID is still running..."
            fi
            sleep 60
